...
class GPTQ:
    ...
    def fasterquant2(self, bias_n, blocksize=128, percdamp=0.01):

        bias_n = bias_n.half()
        W = self.layer.weight.data.clone()
        bias = self.layer.bias.data.clone()
        bias = bias * bias_n
        bias_tn = bias_n.transpose(0, 1)
        W = (W * bias_tn)
        W = W.float()
    ...
    def sacle_k(self, bias_n):
        ...
        return bias, W
    def fasterquant3(self, bias_n, blocksize=128, percdamp=0.01):
